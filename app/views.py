...
@app.route('/indicator/get/<direction>/<data_type>/<int:ttl>')
def indicator_get(direction, data_type, ttl):
    if not (ttl in TTL_VALUES and data_type in DATA_TYPES and direction in DIRECTIONS):
        raise Exception("Bad Values")

    query = db.session.query(Indicator)\
        .filter(Indicator.direction == direction)\
        .filter(Indicator.data_type == data_type)\
        .filter(Indicator.ttl == ttl)
